    ...
    switch (type){
        ...
        case "input-num":
          ...
          case "select":
              ...
              node.id = atributo;
              node.name = atributo;
              new_form.appendChild(la);
              container. appendChild (node);
              for (i=0; i<options.length; i++) {
                  opt = document.createElement("option");
                  opt.innerText = options[i];
                  val = option [i].replace(/\s/g, "").toLowerCase();
                  opt.setAtributte("value", val);
            ...
            case "input-radio":
                ...
                case "input-checkbox":
                    ...
                    atributo = label.trim().toLowerCase;
                    la = document.createElement("label");
                    la.innertHTML = label + "<br>";
                    la.setAtribute("for", atributo);
                    new_form.appendChild(la);
                    for (i=0; i<options.length;i++){
                        opt = document.createElement ("input");
                        val = options[i].replace(/\s/g, "").toLowerCase();
